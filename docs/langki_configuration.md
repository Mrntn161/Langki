...
The donor will receive an API key to unlock this quota. The key is embedded directly in the Langki template, so it works across platforms. The donor must remove the key before sharing their decks.
